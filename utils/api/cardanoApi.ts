...
export async function updateLoan(loanTokenName: string, loanValue: string): Promise<CardanoLoan> {
  const res = await fetch(process.env.NEXT_PUBLIC_API_URL + '/cardano/loan/' + loanTokenName, {
    method: 'PUT',
    headers: {
      'Content-Type': 'application/json',
    },
    body: JSON.stringify({ loanValue: loanValue }),
  })
