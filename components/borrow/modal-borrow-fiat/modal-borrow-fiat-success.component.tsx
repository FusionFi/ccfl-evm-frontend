...
const SuccessMessage = ({ paymentMethod, amount, currency }: any) => {
  ...
  return (
    <>
      <span>
        {t('BORROW_FIAT_MODAL_SUCCESS_BANK_WIRE_MSG')}{' '}
        <span className="modal-borrow-fiat-success-container__status__msg--emphasize">
          {amount} {currency}
        </span>
      </span>
  ...
  return (
    <>
      ...
      <div className="modal-borrow-fiat-success-container__detail__content__item">
        {isUnseal ? (
          <Input
            className="modal-borrow-fiat-success-container__detail__content__item__unseal-box"
            disabled={true}
            defaultValue="https://costco.gift/v612io872g265423g5"
            addonAfter={
              <Tooltip
                ...
                <Button
                  ...
                  size="small"
                  onClick={copyAddress}
                  icon={<CopyOutlined className="text-white" />}></Button>
              </Tooltip>
            }
          />
        ) : (
...
export default function ModalBorrowFiatSuccessComponent({
  ...
  return (
    <Modal
      ...
      title={
        status === TRANSACTION_STATUS.SUCCESS ? t('SUCCESS_MODAL_TITLE') : t('BORROW_MODAL_FAILED')
      }
      open={isModalOpen}
      onOk={_handleCancel}
              ...
              <Link
                ...
                href={
                  selectedNetwork && selectedNetwork.txUrl
                    ? `${selectedNetwork?.txUrl}tx/${txHash}`
                    : `${TX_LINK}${txHash}`
                }
                target="_blank">
